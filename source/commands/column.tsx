...
import {downloadColumn} from '../core/downloader/column-downloader';
...
export default function ColumnCommand({url, flags}: Props) {
	...
	useEffect(() => {
		const run = async () => {
			try {
				...
				const columnId =
					parsed.platform === 'zhihu' && parsed.type === 'column'
						? parsed.columnId
						: url;

				const result = await downloadColumn(columnId, api, client, {
				...
				setSummary(
					`专栏 "${result.folderName}" 下载完成: ${result.success} 成功, ${result.failed} 失败`,
				);
				setIsFinished(true);
	...
	if (error) {
		return (
			<ErrorDisplay
				message={error}
				suggestion="请检查专栏链接是否正确，或运行 zget login 登录后重试"
			/>
		);
	}
